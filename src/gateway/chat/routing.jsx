...
const setApiKey = (key) => {
  if (key) authStorage().setItem(AUTH_KEY, key);
  else authStorage().removeItem(AUTH_KEY);
};
...
async function appFetch(url, options, retried) {
  ...
  let payload = null;
  try { payload = await response.clone().json(); } catch (_error) { /* non-JSON body */ }
  ...
  const replacement = window.prompt(
    'This AxonLLM Sandbox requires a tenant API key.\n' +
