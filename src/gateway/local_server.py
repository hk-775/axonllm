...
def main() -> None:
    """Start the packaged dashboard without pre-bind external diagnostics."""
    ...
    # Live provider diagnostics belong behind the already-authenticated admin
    # route. They must not delay the packaged server from accepting health
    # checks when a provider or AWS catalogue endpoint is unavailable.
