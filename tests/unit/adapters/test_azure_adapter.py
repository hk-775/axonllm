...
class TestTranslateRequest:
    ...
    async def test_basic_request(self, adapter):
        ...
        assert result["model"] == "gpt-4-turbo-2024"
...
class TestTranslateStreamChunk:
    def test_intermediate_chunk(self, adapter):
        ...
        assert chunk.id == "chatcmpl-azure"
        assert chunk.model == "gpt-4-turbo-2024"
...
class TestListModels:
    ...
    async def test_returns_known_models(self, adapter):
        ...
        assert all(m.provider == "azure_openai" for m in models)
        model_ids = [m.model_id for m in models]
        assert "gpt-4-turbo-2024" in model_ids
        assert "gpt-35-turbo" in model_ids
