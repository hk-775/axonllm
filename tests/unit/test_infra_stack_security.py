...
def _cutover_guard_handler(
    ...
    *,
    ...
    source = next(
        ...
        and any(isinstance(target, ast.Name) and target.id == "_RECOVERY_CUTOVER_GUARD" for target in node.targets)
    )
...
def synthesized_template(tmp_path_factory: pytest.TempPathFactory) -> dict:
    ...
    return json.loads((out_dir / "AxonLLMStack.template.json").read_text(encoding="utf-8"))
...
def test_required_inputs_have_no_template_defaults(synthesized_template):
    ...
    assert _REQUIRED_PARAMETERS <= set(parameters)
    for name in _REQUIRED_PARAMETERS:
        assert "Default" not in parameters[name], f"{name} must fail deployment when omitted"
    ...
    assert "PITR validation table" in recovery_table["ConstraintDescription"]
...
def test_deploy_wrapper_requires_and_passes_every_stack_parameter():
    ...
    for parameter, environment_name in _DEPLOY_ENV_BY_PARAMETER.items():
        ...
        assert (f'--parameters "AxonLLMStack:{parameter}=${{{environment_name}}}"') in script
    ...
    assert 'TOPOLOGY_PROFILE="${AXON_TOPOLOGY_PROFILE:-standalone}"' in script
    assert "Deployment profile: ${TOPOLOGY_PROFILE}" in script
    assert "AxonLLM mode: ${TOPOLOGY_PROFILE}" in script
    assert '--parameters "AxonLLMStack:DeploymentMode=$DEPLOYMENT_MODE"' in script
    assert '--parameters "AxonLLMStack:RuntimeStateTableName=$RUNTIME_TABLE_NAME"' in script
    assert '--parameters "AxonLLMStack:RecoveryCutoverMode=$RECOVERY_CUTOVER_MODE"' in script
    assert "require_env AXON_OIDC_ISSUER" in script
    assert "require_env AXON_OIDC_CLIENT_SECRET" in script
    assert 'try_get_context("scim_tenants_secret_arn")' in (_STACK.read_text(encoding="utf-8"))
    assert 'CONTEXT+=(--context "scim_tenants_secret_arn=$SCIM_SECRET_ARN")' in script
...
def test_alb_and_tasks_are_private(synthesized_template):
    load_balancer = _one_resource(synthesized_template, "AWS::ElasticLoadBalancingV2::LoadBalancer")["Properties"]
    ...
    service = _one_resource(synthesized_template, "AWS::ECS::Service")["Properties"]
...
def test_origin_transport_is_tls_only(synthesized_template):
    listener = _one_resource(synthesized_template, "AWS::ElasticLoadBalancingV2::Listener")["Properties"]
    assert listener["Port"] == 443
    assert listener["Protocol"] == "HTTPS"
    assert listener["Certificates"] == [{"CertificateArn": {"Ref": "OriginCertificateArn"}}]
    assert "TLS13-1-2" in listener["SslPolicy"]

    vpc_origin = _one_resource(synthesized_template, "AWS::CloudFront::VpcOrigin")["Properties"][
...
def test_task_egress_and_aws_endpoints_are_explicitly_bounded(
    ...
    task_groups = [
        resource
        for resource in _resources(synthesized_template, "AWS::EC2::SecurityGroup")
    ...
    prefix_list_rules = [
        resource["Properties"]
        for resource in _resources(synthesized_template, "AWS::EC2::SecurityGroupEgress")
        if resource["Properties"].get("Description") == "HTTPS to explicitly approved destinations"
    ]
    assert len(prefix_list_rules) == 1
    rule = prefix_list_rules[0]
    assert rule["Description"] == "HTTPS to explicitly approved destinations"
    assert rule["DestinationPrefixListId"] == {"Ref": "ApprovedHttpsPrefixListId"}
    ...
    assert all(
        ...
        and all(subnet["Ref"].startswith("VpcApplicationSubnet") for subnet in endpoint["SubnetIds"])
        for endpoint in aws_endpoints
    )
    sqs_endpoint = next(endpoint for endpoint in aws_endpoints if endpoint["ServiceName"].endswith(".sqs"))
    sns_endpoint = next(endpoint for endpoint in aws_endpoints if endpoint["ServiceName"].endswith(".sns"))
    logs_endpoint = next(endpoint for endpoint in aws_endpoints if endpoint["ServiceName"].endswith(".logs"))
...
def test_cloudfront_has_tls_waf_and_preserved_behaviors(synthesized_template):
    web_acl = _one_resource(synthesized_template, "AWS::WAFv2::WebACL")["Properties"]
    ...
    distribution = _one_resource(synthesized_template, "AWS::CloudFront::Distribution")["Properties"][
        "DistributionConfig"
    ]
    ...
    assert static[0]["CachePolicyId"] == ("658327ea-f89d-4fab-a63d-7e88639e58f6")
...
def test_alb_and_cloudfront_access_logs_are_private_and_retained(
    ...
    assert properties["OwnershipControls"] == {"Rules": [{"ObjectOwnership": "ObjectWriter"}]}
    ...
    bucket_policy = _one_resource(synthesized_template, "AWS::S3::BucketPolicy")["Properties"]["PolicyDocument"][
        "Statement"
    ]
    assert any(
    ...
    assert any(
        ...
        and statement.get("Principal", {}).get("Service") == "delivery.logs.amazonaws.com"
        and "s3:PutObject" in ([statement["Action"]] if isinstance(statement["Action"], str) else statement["Action"])
        for statement in bucket_policy
    )
...
def test_health_check_and_streaming_settings_are_preserved(
    ...
    target_group = _one_resource(synthesized_template, "AWS::ElasticLoadBalancingV2::TargetGroup")["Properties"]
...
def test_production_oidc_is_conditional_and_bound_to_the_alb(
    ...
    assert identity_rule["RuleCondition"] == {"Fn::Equals": [{"Ref": "DeploymentMode"}, "production"]}
    asserted_parameters = {
        assertion["Assert"]["Fn::Not"][0]["Fn::Equals"][0]["Ref"] for assertion in identity_rule["Assertions"]
    }
    ...
    listener_rule_id = next(
        logical_id for logical_id, resource in synthesized_template["Resources"].items() if resource is listener_rule
    )
    ...
    environment = {item["Name"]: item["Value"] for item in task_definition["ContainerDefinitions"][0]["Environment"]}
    ...
    assert environment["AXON_EXECUTION_TARGET"] == "container"
    ...
    assert synthesized_template["Resources"][primary_table_id]["Type"] == ("AWS::DynamoDB::Table")
    assert environment["AXON_EVENT_OUTBOX_QUEUE_URL"]["Ref"].startswith("SecurityEventOutboxQueue")
    target_group_output = synthesized_template["Outputs"]["TargetGroupArn"]["Value"]
    target_group_id = target_group_output["Ref"]
    assert synthesized_template["Resources"][target_group_id]["Type"] == ("AWS::ElasticLoadBalancingV2::TargetGroup")
...
def test_alb_only_trusts_cloudfront_and_uses_bounded_egress(
    ...
    alb_group = next(
        resource for resource in groups if resource["Properties"]["GroupDescription"].startswith("AxonLLM ALB")
    )
    alb_group_id = next(
        logical_id for logical_id, resource in synthesized_template["Resources"].items() if resource is alb_group
    )
    assert {(rule["IpProtocol"], rule["FromPort"]) for rule in alb_group["Properties"]["SecurityGroupEgress"]} == {
        ("tcp", 53),
        ("udp", 53),
    }
    ...
    cloudfront_ingress = next(
        rule["Properties"]
        for rule in ingress
        if rule["Properties"].get("Description", "").startswith("TLS from the CloudFront")
    )
    assert cloudfront_ingress["FromPort"] == 443
    assert cloudfront_ingress["GroupId"]["Fn::GetAtt"][0] == alb_group_id
    assert cloudfront_ingress["SourcePrefixListId"]["Fn::GetAtt"][1] == ("PrefixLists.0.PrefixListId")
    assert all("CidrIp" not in rule["Properties"] for rule in ingress if rule["Properties"].get("FromPort") == 443)
    ...
    assert https_rule["DestinationPrefixListId"] == {"Ref": "ApprovedHttpsPrefixListId"}
...
def test_private_networking_is_multi_az_and_production_protected(
    ...
    assert attributes["deletion_protection.enabled"] == {"Fn::If": ["ProductionMode", "true", "false"]}
    assert all(subnet["Ref"].startswith("VpcApplicationSubnet") for subnet in load_balancer["Subnets"])
...
def test_optional_public_dns_uses_the_exact_viewer_name(
    ...
    for record in records:
        ...
        assert properties["AliasTarget"]["DNSName"]["Fn::GetAtt"][1] == ("DomainName")
...
def test_state_uses_aws_managed_encryption_and_pitr_without_aws_backup(
    ...
    assert "KmsKeyId" not in secret["Properties"]
    assert "ProviderSecretArn" in synthesized_template["Outputs"]
    assert "DataKeyArn" not in synthesized_template["Outputs"]
    assert "StateBackupVaultArn" not in synthesized_template["Outputs"]
    assert "StateBackupRoleArn" not in synthesized_template["Outputs"]
...
def test_security_event_outbox_is_fifo_encrypted_and_redriven(
    ...
    security_topic = next(
        ...
        if topic["Properties"].get("DisplayName") == "AxonLLM durable security events"
    )
    ...
    assert "KmsKeyId" not in security_log["Properties"]
...
def test_task_role_has_item_permissions_for_atomic_state_transactions(
    ...
    security_log_write = next(
        ...
        and statement["Resource"]["Fn::GetAtt"][0].startswith("SecurityEventLogGroup")
    )
...
def test_runtime_uses_only_the_release_verified_ecr_digest(
    ...
    assert image_parameter["AllowedPattern"].endswith(r"@sha256:[0-9a-f]{64}$")

    task = _one_resource(
        synthesized_template,
        "AWS::ECS::TaskDefinition",
    )["Properties"]
    assert task["ContainerDefinitions"][0]["Image"] == {"Ref": "VerifiedImageUri"}
    assert synthesized_template["Outputs"]["RuntimeImageUri"]["Value"] == {"Ref": "VerifiedImageUri"}
    assert "DataKeyArn" not in synthesized_template["Outputs"]
    selected_table = synthesized_template["Outputs"]["SelectedRuntimeStateTableName"]["Value"]["Fn::If"]
    assert selected_table[:2] == [
        "UseRecoveredState",
        {"Ref": "RuntimeStateTableName"},
    ]
    ...
    authorization = next(statement for statement in statements if statement["Action"] == "ecr:GetAuthorizationToken")
...
def test_recovery_cutover_requires_a_quiesced_service(
    ...
    assert guard["Properties"]["CutoverMode"] == {"Ref": "RecoveryCutoverMode"}
    assert guard["Properties"]["ServiceName"] == "axonllm"
    assert guard["Properties"]["TargetTable"] == {"Ref": "RuntimeStateTableName"}

    service_id, service = next(
        (logical_id, resource) for logical_id, resource in resources.items() if resource["Type"] == "AWS::ECS::Service"
    )
    assert service_id
    assert "RecoveryCutoverGuard" in service["DependsOn"]
    assert service["Properties"]["DesiredCount"] == {"Fn::If": ["RecoveryCutoverActive", 0, 2]}
    ...
    handler = next(
        ...
        if function["Properties"].get("Description") == "Blocks DynamoDB state cutover until AxonLLM is quiesced"
    )
    ...
    guard_policy = next(
        ...
        if any(
            _actions(statement) == {"ecs:DescribeServices"}
            for statement in policy["Properties"]["PolicyDocument"]["Statement"]
        )
    )
    statements = guard_policy["Properties"]["PolicyDocument"]["Statement"]
    describe_service = next(statement for statement in statements if _actions(statement) == {"ecs:DescribeServices"})
    assert (
        _values_for_key(
            describe_service["Resource"],
            "Fn::Join",
        )[0][1][-1]
        == ":service/axonllm/axonllm"
    )
    describe_scaling = next(
        statement
        for statement in statements
        if _actions(statement) == {"application-autoscaling:DescribeScalableTargets"}
    )
...
def test_operational_alarms_are_actionable(synthesized_template):
    ...
    assert len(topics) == 2
    alarm_topic = next(
        topic for topic in topics if topic["Properties"].get("DisplayName") == "AxonLLM production alarms"
    )
    assert "KmsMasterKeyId" not in alarm_topic["Properties"]
    assert _resources(synthesized_template, "AWS::KMS::Key") == []
    ...
    simple_metric_names = {alarm["Properties"].get("MetricName") for alarm in alarms}
