...
class TestRateLimit:
    def test_allows_within_limit(self, enforcer):
        ...
        for _ in range(10):
            result = enforcer.check_rate_limit("proj-1", policy)
            assert result.allowed is True

    def test_blocks_over_limit(self, enforcer):
        policy = ResolvedPolicy(rate_limit_rpm=5)
        for _ in range(5):
            enforcer.check_rate_limit("proj-1", policy)
        result = enforcer.check_rate_limit("proj-1", policy)
    ...
    def test_per_project_isolation(self, enforcer):
        ...
        enforcer.check_rate_limit("proj-a", policy)
        enforcer.check_rate_limit("proj-a", policy)
        # proj-a is at limit
        assert enforcer.check_rate_limit("proj-a", policy).allowed is False
        # proj-b is fresh
        assert enforcer.check_rate_limit("proj-b", policy).allowed is True


class TestBudget:
    def test_allows_within_budget(self, enforcer):
        policy = ResolvedPolicy(budget_limit=100.0)
        enforcer.record_spend("proj-1", 50.0)
        result = enforcer.check_budget("proj-1", 10.0, policy)
        assert result.allowed is True

    def test_blocks_over_budget(self, enforcer):
        policy = ResolvedPolicy(budget_limit=100.0)
        enforcer.record_spend("proj-1", 95.0)
        result = enforcer.check_budget("proj-1", 10.0, policy)
        assert result.allowed is False
    ...
    def test_no_budget_always_allows(self, enforcer):
        ...
        enforcer.record_spend("proj-1", 999999.0)
...
class TestEnforceAll:
    ...
    def test_budget_blocked(self, enforcer):
        ...
        enforcer.record_spend("proj-1", 9.5)
        result = enforcer.enforce_all(
