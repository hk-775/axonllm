...
| **Local development** | `python serve_dashboard.py`. Uvicorn dev server with demo data seeded and `AXON_AUTH_MODE=LOG_ONLY`. | Development |
| **CLI** | `axon serve` (or `axon demo`) after `uv sync`. Same app, argparse front end. | Development, scripted runs |
